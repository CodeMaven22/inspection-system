...
export default function ClientNewInspectionPage() {
    ...
    switch (value) {
      ...
      case "worker_inspection":
        setFormData({
          ...
          inspector_name: "", // This will be assigned by an admin/inspector
          concern_description: "",
          recommendations: "",
          worker_signature: user?.first_name + " " + user?.last_name || "",
          inspector_signature: "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const newInspection: Inspection = {
        id: `insp_${Date.now()}`,
        type: inspectionType,
        status: "draft", // Workers submit as draft for admin approval
        created_at: new Date().toISOString(),
        created_by: user?.id || "unknown",
        submitted_by_role: user?.role || "worker",
        data: formData,
      }

      console.log("Submitting new inspection request:", newInspection)

      toast({
        title: "Inspection Request Submitted",
        description: `Your ${inspectionType.replace(/_/g, " ")} request has been submitted for review.`,
        variant: "default",
      })
      router.push("/client/inspections") // Redirect to client inspections list
    } catch (error) {
      toast({
        title: "Error",
        description: "Failed to submit inspection request. Please try again.",
        variant: "destructive",
      })
    } finally {
      setIsSubmitting(false)
          ...
          <>
            ...
            <div className="grid grid-cols-1 gap-4 md:grid-cols-2">
              ...
              <div className="space-y-2">
                ...
                <Input id="inspector_name" value={workerInspData.inspector_name || ""} onChange={handleChange} />
              </div>
            </div>
            <div className="space-y-2">
              ...
              <div className="flex items-center space-x-2">
                <Checkbox
                  ...
                  onCheckedChange={(checked) =>
                    handleChange({
                      target: {
                        id: "corridros_stairwells_clear_obstructions_storage_combustible",
                        type: "checkbox",
                        checked,
                      },
                    } as any)
  ...
  return (
    <div className="flex flex-col gap-6 p-6">
      <h1 className="text-3xl font-bold tracking-tight">Request New Inspection</h1>
      <p className="text-muted-foreground">Submit a new inspection request for review.</p>

      <Card className="w-full max-w-3xl">
        <CardHeader>
          <CardTitle>Inspection Request Details</CardTitle>
